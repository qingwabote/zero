    ...
    override lateUpdate(): void {
        ...
        for (const slot of this._skeleton.drawOrder) {
            ...
            if (attachment instanceof sc.RegionAttachment) {
                attachment.computeWorldVertices(slot, vertexBuffer.data, VERTEX_ELEMENTS * vertex, VERTEX_ELEMENTS);
                ...
                attachment.computeWorldVertices(slot, 0, attachment.worldVerticesLength, vertexBuffer.data, VERTEX_ELEMENTS * vertex, VERTEX_ELEMENTS);
            ...
            if (attachment_texture) {
                const uv_p = VERTEX_ELEMENTS * vertex + FormatInfos[a_position.format].nums;
                for (let i = 0; i < attachment_vertex; i++) {
                    const j = VERTEX_ELEMENTS * i + uv_p;
                    const k = 2 * i;
                    vertexBuffer.data[j] = attachment_uvs[k];
                    vertexBuffer.data[j + 1] = attachment_uvs[k + 1];
                }
                indexBuffer.set(attachment_triangles.map(i => i + vertex), index);
