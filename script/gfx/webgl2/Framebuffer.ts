...
export class Framebuffer {
    ...
    initialize(info: FramebufferInfo): boolean {
        ...
        const isDefaultFramebuffer = (info.colorAttachments as Vector<Texture>).data.find(texture => texture.swapchain) != undefined;
        ...
        for (let i = 0; i < (info.colorAttachments as Vector<Texture>).data.length; i++) {
            const attachment = (info.colorAttachments as Vector<Texture>).data[i];
        ...
        const depthStencilAttachment = info.depthStencilAttachment;
