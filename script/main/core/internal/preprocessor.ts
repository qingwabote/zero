import { ShaderStage } from "../gfx/Shader.js";
...
export default {
    ...
    async preprocess(stages: Readonly<ShaderStage[]>, macros: Readonly<Record<string, number>>): Promise<Readonly<ShaderStage[]>> {
        return stages.map(stage => ({ type: stage.type, source: macroExpand(macros, stage.source) }));
    }
}