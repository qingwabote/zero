...
export class SkinAlive {
    ...
    constructor(_proto, _root) {
        this._proto = _proto;
        this._root = _root;
        this._offset = new Periodic(-1, -1);
        this._joints = _proto.joints.map(paths => _root.getChildByPath(paths));
    }
    update() {
        ...
        for (let i = 0; i < this._joints.length; i++) {
            mat4.multiply_affine(mat4_a, toModelSpace(this._root, this._joints[i]), this._proto.inverseBindMatrices[i]);
