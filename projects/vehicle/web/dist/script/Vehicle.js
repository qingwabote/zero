import { Component, GLTF, MeshRenderer, Node, bundle, quat, vec3, vec4 } from "engine";
...
export default class Vehicle extends Component {
    ...
    constructor(node) {
        ...
        this.addWheel(vehicle, vec3.create(wheel_halfTrack, wheel_axis_height, wheel_axis_front_position), tuning, true);
        this.addWheel(vehicle, vec3.create(-wheel_halfTrack, wheel_axis_height, wheel_axis_front_position), tuning, true);
        this.addWheel(vehicle, vec3.create(wheel_halfTrack, wheel_axis_height, wheel_axis_back_position), tuning, false);
        this.addWheel(vehicle, vec3.create(-wheel_halfTrack, wheel_axis_height, wheel_axis_back_position), tuning, false);
        PhysicsSystem.instance.world.impl.addAction(vehicle);
        this._impl = vehicle;
    }
    setEngineForce(value, wheel) {
        this._impl.applyEngineForce(-value, wheel);
    }
    setBrake(value, wheel) {
        this._impl.setBrake(value, wheel);
    }
    setSteeringValue(value, wheel) {
    ...
    addWheel(impl, pos, tuning, isFront) {
